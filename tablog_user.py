...
filename_list = [name.split("\\")[-1] for name in glob("tablog_userdata/*.json")]
data = file_load("user.json")
start_time = time.time()
target_username = "まみのる"
for url,user in data.items():
    ...
    for j in range(1,41):
        ...
        print(user,j)
        try:
            file_create(data_list,f"tablog_userdata/{user}-{j}.json")
        except:
            file_create(data_list,f"tablog_userdata/{user}-{j}.json")
    end_time = time.time()
