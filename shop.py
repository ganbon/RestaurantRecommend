...
class Shop:
    def __init__(self,shop_file_path=None,shop_data=None):
        ...
        if "word" in list(self.shop_data.keys()):
    ...
    def word_select(self):
        ...
        comment_word = df['word'].value_counts().head(30).index.tolist()
        return comment_word
    ...
    def vector_concat(self):
        ...
        if len(self.comment_word_vector.keys()) < 10:
            self.shop_vector = np.concatenate([self.shop_vector,np.zeros((10-len(self.comment_word_vector.keys()),200))])
        self.shop_data["shop_vector"] = self.shop_vector.tolist()

    # とりあえず置いてる
    def save_json(self,file_name,genre_op=True,vector=True):
